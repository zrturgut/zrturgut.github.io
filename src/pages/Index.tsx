  ...
  return (
    <motion.div
      ...
      className={`relative overflow-hidden cursor-pointer transition-all duration-500 ease-in-out
        ${isActive ? "flex-[3]" : "flex-1 hover:flex-[1.5]"}
    ...
    >
      ...
      <div className={`relative h-full flex flex-col p-8 z-20 ${isActive ? 'justify-start' : 'justify-end items-center text-center'}`}>
          ...
          >
            <div className="space-y-12">
              {resumeData.projects.map((project, i) => (
                <div key={i} className="group relative overflow-hidden rounded-xl border border-white/10 hover:border-purple-500/50 transition-all duration-300">
                  {project.gif && (
                    <div className="absolute inset-0 z-0">
                      <div className="absolute inset-0 bg-black/90 group-hover:bg-black/40 transition-colors duration-500 z-10" />
                      <img src={project.gif} alt={project.title} className="w-full h-full object-cover opacity-20 group-hover:opacity-60 transition-opacity duration-500" />
                    </div>
                  )}
                  <div className="relative z-20 p-6">
                    <div className="flex justify-between items-start mb-4">
                      ...
                      {project.subtitle && <span className="text-xs font-mono px-2 py-1 bg-purple-500/20 text-purple-300 rounded backdrop-blur-md">{project.subtitle}</span>}
                    </div>
                    <ul className="space-y-2 text-gray-400 group-hover:text-gray-200 transition-colors">
          ...
          >
            <div className="space-y-10">
              {resumeData.experience.map((exp, i) => (
                <div key={i} className="border-l-2 border-purple-500/30 pl-6 ml-2 relative group hover:pl-8 transition-all duration-300">
                  <div className="absolute -left-[9px] top-0 w-4 h-4 rounded-full bg-black border-2 border-purple-500 group-hover:scale-125 transition-transform" />
                  <div className="mb-2">
                    <h3 className="text-xl font-bold text-white">{exp.role}</h3>
                    <div className="text-purple-400 font-mono text-sm">{exp.company} | {exp.period}</div>
                  </div>
                  <ul className="space-y-2 text-gray-400 mt-4 group-hover:text-gray-300">
          ...
          >
            <div className="space-y-8">
              {resumeData.education.map((edu, i) => (
                <div key={i} className="bg-white/5 p-6 rounded-lg border border-white/5 hover:bg-white/10 transition-colors backdrop-blur-sm">
